...
get_header(); ?>
	<div class="fullwidth mid-content"> <!-- Middle content align -->
		...
		while ( have_posts() ) : the_post();
			...
			$video_catwalk = ($custom['video_catwalk'][0]);
			$tabs_content = '<ul class="tabs"><li class="active"><a href="#" data-tab="gallery">Galerie</a></li>';
			if(sizeof($custom['gallery_polas']) > 0 ) {
				$tabs_content .= '<li><a href="#" data-tab="gallery_polas">Polas</a></li>';
			}
			if($video ){
				$tabs_content .= '<li><a href="#" data-tab="video">Video</a></li>';
			}
			if($video_catwalk ){
				$tabs_content .= '<li><a href="#" data-tab="video_catwalk">Video (Laufsteg)</a></li>';
			}
			...
			if(sizeof($custom['gallery_polas']) > 0 ) {
				$tabs_content.='<li><a class="download_polas_sedcard" href="'.get_permalink().'?pdf_download=true&polas=true">Download (Polas)</a></li>';
			}
			$tabs_content.='</ul>';
			...
			if($_GET['pdf_download']){
				...
				echo '<div class="post_single_page_content_wrapper item '.esc_attr($selected).'" id="'.get_the_ID().'">'; // Post Data content wrapper note:don't delete this ID and class
					...
					if(substr($custom['groesse'][0], -3) == ' cm'){
						$custom['groesse'][0] = substr($custom['groesse'][0], 0, -3);
					}
					if(substr($custom['chest'][0], -3) == ' cm'){
						$custom['chest'][0] = substr($custom['chest'][0], 0, -3);
					}
					...
					if(sizeof($custom['gallery_polas']) > 0  ) {
						echo '<div class="tab_content tab_gallery_polas gallery">';
						$i = 0;
						foreach ($custom['gallery_polas'] as $image_id){
							$image_attributes_thumb =  wp_get_attachment_image_src( $image_id, $size = 'medium_large' ) ;
							$image_attributes_full =  wp_get_attachment_image_src( $image_id , $size = 'large') ;
							if(sizeof($custom['gallery_polas']) == 1  ) {
								$classname = "single";
							}else {
								$classname = "";
							}
					...
					if($video_catwalk){
						echo '<div class="tab_content tab_video_catwalk">';
						
						foreach($custom['video_catwalk'] as $video){
							$u= '<video width="630" height="354" controls>
								  <source src="'.wp_get_attachment_url( $video ) .'" type="video/mp4">							  
								Your browser does not support the video tag.
								</video>'; 
	...
	<div style="visibility:hidden">
		<?php //new dBug(sizeof($custom['gallery_polas'])); ?>
	</div>
<?php get_footer(); ?>
